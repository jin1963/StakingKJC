...
async function refreshWalletStatus() {
  ...
  try {
    ...
    console.error("refreshWalletStatus error:", e);
  }
}
  ...
  try {
    ...
    while (true) {
      try {
        ...
        const pendingRaw = await contract.methods.pendingReward(user, currentIndex).call().catch(() => "0");
        ...
        card.innerHTML = `
          <p><strong>Index:</strong> ${currentIndex}</p>
        ...
        if (!stake.claimed && canClaim) {
          ...
          claimBtn.onclick = async () => {
            try {
              await ensureChain();
              await contract.methods.claim(currentIndex).send({ from: user });
              alert("✅ Claimed");
              await refreshWalletStatus();
              await loadStakes();
            } catch (e) {
        ...
        if (!stake.claimed && canUnstake) {
          ...
          unstakeBtn.onclick = async () => {
            try {
              await ensureChain();
              await contract.methods.unstake(currentIndex).send({ from: user });
              alert("✅ Unstaked");
              await refreshWalletStatus();
              await loadStakes();
            } catch (e) {
