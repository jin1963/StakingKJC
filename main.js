...
async function loadStakes() {
  ...
  while (true) {
    try {
      ...
      const start = stake.startTime && stake.startTime > 0
        ? new Date(Number(stake.startTime) * 1000).toLocaleDateString("th-TH")
        : "-";

      const unlock = (stake.lockPeriod && stake.lockPeriod > 0)
        ? new Date((Number(stake.startTime) + Number(stake.lockPeriod) * 86400) * 1000).toLocaleDateString("th-TH")
        : "-";
